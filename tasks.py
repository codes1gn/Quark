...
@task
@check_framework
def bootstrap(ctx, framework=None):
    """
...
def link_config_file(ctx, framework=None):
    ...
    relative_path = f"./quark/environments/configs/py{pyver_str}/{framework}/pyproject.toml"
...
def build(ctx, framework=None):
    ...
    with ctx.prefix(activate_cmd):
        ...
        #     ctx.run("pip install numpy==1.23.4")
        ctx.run("poetry lock")
...
@task
def bootstrap_torch(ctx):
    """Bootstrap a PyTorch environment."""
    bootstrap(ctx, framework="torch")

@task
def bootstrap_tensorflow(ctx):
    """Bootstrap a TensorFlow environment."""
    bootstrap(ctx, framework="tensorflow")

@task(pre=[bootstrap_torch])
def build_torch(ctx):
    """Build the project in the PyTorch environment."""
    build(ctx, framework="torch")

@task(pre=[bootstrap_tensorflow])
def build_tensorflow(ctx):
    """Build the project in the TensorFlow environment."""
    build(ctx, framework="tensorflow")
