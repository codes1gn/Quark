...
class TorchDataProvider(DataProviderBase):
    ...
    def generate_synthetic_data(self):
        ...
        DEBUG(
            "self.input_shape = {} with type = {}".format(
                self.input_shape, type(self.input_shape)
            )
        )
        assert isinstance(self.batch_size, int)
        assert isinstance(self.input_shape, (tuple, list)) and all(
            isinstance(x, int) for x in self.input_shape
        )
        # TODO: compare and determine the rng method
        inputs_data = np.random.rand(self.batch_size, *self.input_shape).astype(
            self.data_type.to_numpy()
        )
    ...
    def load_dataset(self, config: BenchmarkConfig):
        ...
        if self.data_source == DataSourceEnum.SYNTHETIC:
            ...
            for input, label in self.dataset:
                TRACE(
                    "Dataset = MNIST; Input Shape = {}; Label Shape = {}".format(
